...
namespace FerramAerospaceResearch.FARAeroComponents
{
    public class FARVesselAero : VesselModule
    {
        ...
        private void CalculateAndApplyVesselAeroProperties()
        {
            ...
            float pseudoKüchemannNumber = (float)(machNumber / (reynoldsNumber + machNumber));
            ...
            for (int i = 0; i < _currentAeroSections.Count; i++)
                _currentAeroSections[i].FlightCalculateAeroForces(atmDensity, (float)machNumber, (float)(reynoldsNumber / Length), pseudoKüchemannNumber, skinFrictionDragCoefficient);
        ...
        public void SimulateAeroProperties(out Vector3 aeroForce, out Vector3 aeroTorque, Vector3 velocityWorldVector, double altitude)
        {
            ...
            float pseudoKüchemannNumber = machNumber / (reynoldsNumber + machNumber);

            for(int i = 0; i < _currentAeroSections.Count; i++)
                _currentAeroSections[i].PredictionCalculateAeroForces(density, machNumber, reynoldsPerLength, pseudoKüchemannNumber, skinFriction, velocityWorldVector, center);
